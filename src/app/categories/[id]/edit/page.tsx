...
export default function EditCategoryPage() {
  ...
  useEffect(() => {
    if (!params.id) return;
    fetch(`/api/categories/${params.id}`).then(async (r) => {
      if (r.ok) {
        const data = await r.json();
        setForm({ name: data.name, type: data.type, icon: data.icon, color: data.color });
        setLoading(false);
      }
    });
  }, [params.id]);
  ...
  return (
    <div className="p-4 md:p-6 max-w-lg mx-auto">
      <h1 className="text-[15px] md:text-[18px] font-semibold -tracking-[0.01em] text-white mb-6">Edit Category</h1>
